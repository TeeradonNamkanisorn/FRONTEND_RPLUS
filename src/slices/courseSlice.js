...
const fetchCourseAsync = createAsyncThunk('course/fetch', async(payload, thunkAPI) => {
    try {
        ...
        return thunkAPI.rejectWithValue(err?.response?.data?.message || "request error")
    }
})

//only for student mode
const fetchStudentCourseAsync = createAsyncThunk('studentCourse/fetch', async (payload, {rejectWithValue}) => {
    try {
        const {courseId} = payload;
        const res = await axios.get('/course/as-student/'+courseId);
        return res.data.course
    } catch (err) {
        return rejectWithValue(err.response?.data?.message || err.message || "request error")
    }
})

//The mark complete counterpart is defined after the create slice.
const unmarkCompleteAsync = createAsyncThunk('lesson/unmark', async (payload, {rejectWithValue}) => {
    try {
...
const remarkCompleteAsync = createAsyncThunk('lesson/remark', async ({lessonId}, {rejectWithValue}) => {
    try {
...
const courseSlice = createSlice({
    ...
    extraReducers: builder => {
        ...
        .addCase(fetchCourseAsync.fulfilled, (state, action) => {
            state = {
                ...action.payload, isLoading: false, error: ""
            }
            return state
        })
        .addCase(
        ...
        .addCase(
            fetchStudentCourseAsync.pending, (state, action) => {
                state.isLoading = true;
                state.error = "";
            })
        ...
        .addCase(remarkCompleteAsync.fulfilled, (state,action) => {
            ...
            const idx = state.previouslyCompletedLessons.find(el => el === lessonId);
...
const {setLoading: setCourseLoading, setError, addCompletedLesson} = courseSlice.actions;
...
function markLessonCompleteAsync({lessonId}) {
    return async (dispatch,getState) => {
        try {
            ...
            dispatch(setError(err.response?.data?.message || err.message || "request error"));
        } finally {
...
export {fetchCourseAsync, setCourseLoading, fetchStudentCourseAsync, markLessonCompleteAsync, unmarkCompleteAsync, remarkCompleteAsync};
