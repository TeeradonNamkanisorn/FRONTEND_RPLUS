...
function CoursePreview() {
  ...
  console.log(Math.round(course.totalLength));
  const length = secondsToHm(Math.round(course.totalLength));
  const { courseId } = useParams();
  const isInCart = Boolean(useSelector((state) => state.cart.cart[courseId]));

  const { price, name, description, imageLink, level, videoLink, updatedAt } =
    course;
  useEffect(() => {
